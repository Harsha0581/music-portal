...
try{
	  ...
	  String qry1="select film_id,title,producer,director,description,publish_date,duration,language,short_film_video_pic,views,likes,status,film_file from short_films where username='"+session.getAttribute("username")+"'";
	  ...
	  if(rs.next())
	  {
		...
		do
		{
			...
			out.println("<th style='background-color:black'><A class='btn btn-sm btn-primary' href='view_short_film_edit_code.jsp?film_id="+rs.getString("film_id")+"'>Edit</th> <th><A class='btn btn-sm btn-danger' href='view_short_film_code.jsp?film_id="+rs.getString("film_id")+"'>Delete</A></th>");					
				...
				%>
				<td>
        		<A href='play_song.jsp?url=user_view_songs.jsp&song_id=<%=rs.getString("film_id")%>&song=<%=path+"/uploads/"+rs.getString("film_file")%>' class='btn btn-sm btn-success'>PLAY VIDEO</A>
