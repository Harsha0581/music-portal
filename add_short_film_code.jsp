	...
	if(! ProjectDir.exists()) // if uploads directory does not exist in the application 
	{
       ...
       while (itr.hasNext())
       {
		  ...
		  */
			...
			String qry="insert into short_films (title,producer,director,description,publish_date,duration,language,short_film_video_pic,film_file,username) values ("+"'"+colvalues.get(0)+"',"+"'"+colvalues.get(1)+"',"+"'"+colvalues.get(2)+"',"+"'"+colvalues.get(3)+"',"+"'"+colvalues.get(4)+"',"+"'"+colvalues.get(5)+"',"+"'"+colvalues.get(6)+"',"+"'"+colvalues.get(7)+"',"+"'"+colvalues.get(8)+"','"+session.getAttribute("username")+"')";	 
try{
...
Class.forName(JDBC_DRIVER);
	  ...
	  if(n>0)
		  out.println("<script language='javascript'>window.alert('Record Added Successfully.');window.location='add_short_film.jsp';</script>");
	  else
		  out.println("<script language='javascript'>window.alert('Record Not Added.  Try Again!');window.location='add_short_film.jsp';</script>");
