	...
	if(! ProjectDir.exists()) // if uploads directory does not exist in the application 
	{
       ...
       while (itr.hasNext())
       {
		  ...
		  */
			...
			String qry="insert into advertisement (poster_pic,poster_date,status) values ("+"'"+colvalues.get(0)+"',"+"'"+colvalues.get(1)+"',"+"'"+colvalues.get(2)+"')";	 
try{
...
Class.forName(JDBC_DRIVER);
	  ...
	  if(n>0)
		  out.println("<script language='javascript'>window.alert('Record Added Successfully.');window.location='add_advertisement.jsp';</script>");
	  else
		  out.println("<script language='javascript'>window.alert('Record Not Added.  Try Again!');window.location='add_advertisement.jsp';</script>");
